...
seurat_filtered = readRDS(here("R/results/h225_5_seurat_integrated_rpca_FILTERED.RDS"))
...
p3a = DimPlot(seurat_filtered, label = T, repel = T, group.by = "rpca_clusters.v2", raster = F, shuffle = T, reduction = "umap.rpca", split.by = "species")

p4a = DimPlot(seurat_filtered, label = T, repel = T, group.by = "uniform_name_overview2", raster = F, shuffle = T, reduction = "umap.rpca", split.by = "species")
...
p3b = DimPlot(seurat_filtered_purecells, label = T, repel = T, group.by = "rpca_clusters.v2", raster = F, shuffle = T, reduction = "umap.rpca", split.by = "species")

p4b = DimPlot(seurat_filtered_purecells, label = T, repel = T, group.by = "uniform_name_overview2", raster = F, shuffle = T, reduction = "umap.rpca", split.by = "species")

p3b+p4b + plot_annotation(title = "7601 5.5% cells with ambiguos annotation excluded")

plotly::ggplotly( DimPlot(seurat_filtered_purecells[, seurat_filtered_purecells$rpca_clusters.v2==22], label = T,   group.by = "uniform_name_overview2", raster = F, shuffle = T, reduction = "umap.rpca", split.by = "species"))
...
DimPlot(seurat_filtered_purecells[,seurat_filtered_purecells$uniform_name_overview2 %in% c("Platelet", "Megakaryocyte", "Neutrophils")], group.by = "species_celltype",reduction = "umap.rpca" , shuffle = T)
...
p_celltypes2 = DimPlot(neutrosubset, group.by = c("uniform_name_overview2",'experiment') ,reduction = "umap.rpca" , shuffle = T,  raster = F, label = T, split.by =  "species")
p_celltypes2

p_celltypes3 = DimPlot(neutrosubset, group.by = c("uniform_name_overview2",'rpca_clusters.v2', "cluster_labels_res.0.8") ,reduction = "umap.rpca" , shuffle = T,  raster = F, label = T, split.by =  "species")
p_celltypes3


FeaturePlot(neutrosubset, features = c('CXCR4','CXCR2'), split.by = "species", reduction = "umap.rpca") # ist auch CXCR2 low in https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7442692/ beschrieben fuer immatures , CXCR4 high
...
neutro_integrated = DimPlot(neutrosubset_human, group.by = c("cluster_labels_res.0.8", "uniform_name_overview2"),reduction = "umap.rpca"  )
...
DimPlot(neutrosubset_human, group.by = c("cluster_labels_res.0.8", "rpca_clusters"), reduction = "umap.rpca.v2", label = T)
...
p_celltypes0 = DimPlot(seurat_filtered_purecells, group.by = c("uniform_name_overview3"),label = T, repel = T, raster = F, shuffle = T, reduction = "umap.rpca")
p_celltypes0

p_celltypes = DimPlot(seurat_filtered_purecells, group.by = c("uniform_name_overview3"),label = T, repel = T, raster = F, shuffle = T, reduction = "umap.rpca.v2")
p_celltypes

p_celltypes0 + p_celltypes
...
p_species_after = DimPlot(seurat_filtered_purecells, group.by = c("species"), repel = T, raster = F, shuffle = T, reduction = "umap.rpca")
...
p_celltypes4 = DimPlot(neutrosubset2, group.by = c("rpca_clusters.v2","uniform_name_overview3") ,reduction = "umap.rpca" , shuffle = T,  raster = F, label = T, split.by =  "species")
p_celltypes4


FeaturePlot(neutrosubset, features = c('ELANE', 'FUT4', "SPN" ,'CEACAM8', "PADI4"), split.by = "species",reduction = "umap.rpca" ) # ELANE FUT4 spn Pro-Neu, PADI4 Pre-Neu in schulte schrepping f cells with a proliferative signature  # Neutrophils from COVID-19, particularly from patients with severe disease, primarily occupied immature pre- and pro-neutrophil-like clusters.  The TF network underlying the transcriptional difference in pro-neutrophils is mainly driven by E2F family members and pre-neutrophils mainly depend on ETS TFs (Figure S6H).
...
p_celltypes5 = DimPlot(cd4subset2, group.by = c("rpca_clusters.v2","uniform_name_overview3", "severity") ,reduction = "umap.rpca" , shuffle = T,  raster = F, label = T, split.by =  "species")
p_celltypes5

p_celltypes5b = DimPlot(cd4subset2, group.by = c("rpca_clusters.v2","uniform_name_overview3") ,reduction = "umap.rpca" , shuffle = T,  raster = F, label = T, split.by =  "species")
p_celltypes5b

# speichern ###
jpeg2(here("R/results/h228_2_umap_before_after_integration_species.jpeg"), 11,5)
p_species_before + p_species_after + plot_layout(guides = "collect") + plot_annotation(caption = "h228_2_define_pure_celltypes_per_cluster_for_export.R")
dev.off()

jpeg2(here("R/results/h228_2_umapcelltypes_allspecies.jpeg"), 9,6)
p_celltypes + plot_annotation(caption = "h228_2_define_pure_celltypes_per_cluster_for_export.R")
dev.off()

fwrite(cellanno_filtered_purecells, here("R/results/h228_2_qc_integrated_rpca_cellanno_FILTEREDqc_FILTEREDpureCells.txt.gz"), sep = "\t")



saveRDS(seurat_filtered_purecells, here("R/results/h228_2_qc_integrated_rpca_cellanno_FILTEREDqc_FILTEREDpureCells.rds"))
## finalize script ----
finalizeSkript()
